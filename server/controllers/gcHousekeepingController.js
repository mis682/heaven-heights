...
exports.createSubmission = async (req, res) => {
  ...
  const messageLines = [
    `🧹 *${submittedBy}* submitted *${form.label}* (Garden City Housekeeping) — checkpoints ${form.checkpointStart}-${form.checkpointEnd}`,
    "",
    ...photos.map((p) => `Checkpoint ${p.checkpointId}: ${p.photoUrl}`),
  ];
