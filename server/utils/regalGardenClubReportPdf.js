...
function buildRegalGardenClubReportPdf(report, form) {
  ...
  const title = `Regal Garden Club — ${form?.label || `Form ${report.formNumber}`} — ${report.reportDate}`;

  doc.font("Helvetica-Bold").fontSize(12).fillColor("#111827").text(title, MARGIN, MARGIN - 10);
  ...
  report.entries.forEach((e) => {
    ...
    drawDataRow(doc, [e.checkpointLabel, e.status || "—"], MARGIN, y);
    y += ROW_HEIGHT;
  });
...
module.exports = { buildRegalGardenClubReportPdf };
