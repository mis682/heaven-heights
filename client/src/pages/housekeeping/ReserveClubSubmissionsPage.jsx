...
function formLabel(formNumber) {
  return RESERVE_CLUB_FORMS.find((f) => f.formNumber === formNumber)?.label || `Form ${formNumber}`;
}

export default function ReserveClubSubmissionsPage() {
  ...
  const copyLink = async (formNumber) => {
    const url = `${window.location.origin}/reserve-club-form/${formNumber}`;
  ...
  return (
    <div>
      <PageHeader
        title="Neoteric Reserve Club — Submissions"
        subtitle="Checkpoint photo responses across all Neoteric Reserve Club forms."
      />
      ...
      <DataTable
        columns={[
          ...
          { key: "photoCount", header: "Photos" },
          {
            ...
            render: (r) => (
              <button
                onClick={async () => setViewing(await getReserveClubSubmission(r._id))}
        ...
        emptyHint={loading ? "" : "No Neoteric Reserve Club submissions yet"}
      />
      ...
      {viewing && lightboxIndex !== null && (
        <PhotoLightbox
          ...
          downloadName={(p) => `reserve-club-${p.checkpointLabel}`}
        />
      )}
    </div>
  );
