...
import {
  ...
  getReserveClubReportByDate,
  saveReserveClubReportDraft,
  submitReserveClubReport,
...
// Same pattern as GCClubDailyReportPage — a form picker (one per Reserve
// Club form) plus a date picker; the selected form+date's checkpoints all
// appear at once with a status dropdown per row.
export default function ReserveClubDailyReportPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    getReserveClubReportMeta().then((m) => {
      setMeta(m);
  ...
  useEffect(() => {
    ...
    (async () => {
      const r = await getReserveClubReportByDate(formNumber, date);
      setReport(r);
      if (r) {
  ...
  const persist = async (targetStatus) => {
    setSaving(true);
    const saved = await saveReserveClubReportDraft({
      formNumber,
      reportDate: date,
      entries,
    ...
    if (targetStatus === "submitted") {
      try {
        const submitted = await submitReserveClubReport(saved._id);
        setReport(submitted);
  ...
  if (meta.forms.length === 0) {
    return (
      <div>
        <PageHeader title="Neoteric Reserve Club — Daily Report" subtitle="Koi form abhi tak add nahi hua hai." />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        title="Neoteric Reserve Club — Daily Report"
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        {report && (
          <>
            <a
              href={reserveClubReportExportPdfUrl(report._id)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              <FileText size={16} /> Download PDF
            </a>
            <a
              href={reserveClubReportExportUrl(report._id)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
