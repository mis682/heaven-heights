...
function percentClasses(pct) {
  if (pct >= 90) return "bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-400";
  if (pct >= 70) return "bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-400";
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <label className="text-sm text-subtext dark:text-gray-400 flex items-center gap-2">
          From
          <ThemedDatePicker value={from} max={to} onChange={setFrom} />
        </label>
        <label className="text-sm text-subtext dark:text-gray-400 flex items-center gap-2">
          To
          <ThemedDatePicker value={to} max={today()} onChange={setTo} />
        </label>
        {!loading && (
          ...
          <table className="text-sm w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900/40 border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="text-right px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">No Photo</th>
                <th className="text-right px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">On-Time %</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  {expanded === r.guardName && r.lateDetails.length > 0 && (
                    <tr>
                      <td colSpan={7} className="bg-gray-50 dark:bg-gray-900/40 px-4 py-3">
                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Late checkpoints</p>
                          ...
                          {r.lateDetails
                            ...
                            .map((d, i) => (
                              <div key={i} className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-300">
                                ...
                                <span className="w-16 shrink-0 font-medium text-heading dark:text-gray-100">{d.checkpointLabel}</span>
