...
import { getNightGuardGuardKpi } from "../../../api/nightguard";
...
function downloadCsv(rows, from, to) {
  const header = ["Guard Name", "Total Check-ins", "On Time", "Late", "Missed", "On-Time %"];
  ...
  a.href = url;
  a.download = `night-guard-kpi-${from}-to-${to}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

export default function NightGuardGuardKpiPage() {
  ...
  useEffect(() => {
    setLoading(true);
    getNightGuardGuardKpi(from, to).then((r) => {
      setRows(r.rows);
      setLoading(false);
    });
  }, [from, to]);
  ...
  return (
    <div>
      <PageHeader
        title="Night Guard — Guard KPI"
        subtitle="Har guard se roughly har ghante ek check-in expect hota hai — 60 min ke andar continue karna on time hai, usse zyada gap hone par Late/Missed count hota hai."
        primaryAction={
                  ...
                  {expanded === r.guardName && r.lateDetails.length > 0 && (
                    <tr>
                      <td colSpan={7} className="bg-gray-50 dark:bg-gray-900/40 px-4 py-3">
                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Late / missed check-ins</p>
                            ...
                            .map((d, i) => (
                              <div key={i} className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-300">
                                {d.lateByMinutes == null ? (
                                  <>
                                    <span className="w-44 shrink-0">Before {formatTime(d.before)}</span>
                                    <span className="text-gray-500 dark:text-gray-400 font-medium">Missed — no check-in</span>
                                  </>
                                ) : (
                                  <>
                                    <span className="w-44 shrink-0">Check-in at {formatTime(d.before)}</span>
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-subtext dark:text-gray-400">
                    Is date range mein koi check-in nahi mila.
                  </td>
                </tr>
              )}
            </tbody>
