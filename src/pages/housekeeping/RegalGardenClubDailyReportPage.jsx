...
import {
  ...
  getRegalGardenClubReportByDate,
  saveRegalGardenClubReportDraft,
  submitRegalGardenClubReport,
...
// Same pattern as ReserveClubDailyReportPage — a form picker (one per Regal
// Garden Club form) plus a date picker; the selected form+date's
// checkpoints all appear at once with a status dropdown per row.
export default function RegalGardenClubDailyReportPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    getRegalGardenClubReportMeta().then((m) => {
      setMeta(m);
  ...
  useEffect(() => {
    ...
    (async () => {
      const r = await getRegalGardenClubReportByDate(formNumber, date);
      setReport(r);
      if (r) {
  ...
  if (meta.forms.length === 0) {
    return (
      <div>
        <PageHeader title="Regal Garden Club — Daily Report" subtitle="Koi form abhi tak add nahi hua hai." />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        title="Regal Garden Club — Daily Report"
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        {report && (
          <>
            <a
              href={regalGardenClubReportExportPdfUrl(report._id)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
              <FileText size={16} /> Download PDF
            </a>
            <a
              href={regalGardenClubReportExportUrl(report._id)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
