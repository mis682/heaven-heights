...
import {
  getRegalGardenClubReportMeta,
  listSubmittedRegalGardenClubReports,
  getRegalGardenClubReport,
  unlockRegalGardenClubReport,
  regalGardenClubReportExportUrl,
  regalGardenClubReportExportPdfUrl,
} from "../../api/regalGardenClubReport";

export default function RegalGardenClubAdminReportPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    getRegalGardenClubReportMeta().then(setMeta);
  }, []);

  const load = async () => {
    setLoading(true);
    const data = await listSubmittedRegalGardenClubReports({
  ...
  const view = async (id) => setViewing(await getRegalGardenClubReport(id));

  const handleUnlock = async (id) => {
    await unlockRegalGardenClubReport(id);
    setViewing(null);
    load();
  };
  ...
  return (
    <div>
      <PageHeader title="Regal Garden Club — Admin Report View" subtitle="Submitted daily reports, read-only." />
          ...
          { key: "submittedAt", header: "Submitted At", render: (r) => new Date(r.submittedAt).toLocaleString() },
          {
            ...
            render: (r) => (
              <div className="flex items-center gap-3">
                ...
                <a href={regalGardenClubReportExportUrl(r._id)} className="text-xs font-semibold text-gray-600 dark:text-gray-400 hover:underline inline-flex items-center gap-1">
                  <Download size={12} /> Excel
                </a>
                <a href={regalGardenClubReportExportPdfUrl(r._id)} className="text-xs font-semibold text-gray-600 dark:text-gray-400 hover:underline inline-flex items-center gap-1">
      ...
      {viewing && (
        <Modal title={`Regal Garden Club — ${viewing.reportDate}`} onClose={() => setViewing(null)} wide>
          ...
          <div className="flex items-center gap-2">
            <a
              href={regalGardenClubReportExportPdfUrl(viewing._id)}
            ...
            <a
              href={regalGardenClubReportExportUrl(viewing._id)}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
