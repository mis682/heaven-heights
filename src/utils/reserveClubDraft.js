// Same as utils/gcClubDraft.js — keyed by checkpoint label, since Neoteric
// Reserve Club's forms use named checklists too.
const PREFIX = "reserve-club-draft-";
...
export async function saveDraft(formNumber, submittedBy, captures, textAnswers) {
  try {
    ...
    console.warn("Could not save Reserve Club draft", e);
  }
}
...
export function loadDraft(formNumber) {
  try {
    ...
    console.warn("Could not load Reserve Club draft", e);
    return null;
  }
}
